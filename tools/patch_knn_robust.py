...
p = Path('services/geometry-api/web/market-knn.js')
...
start = s.index('  function spatialKnnEstimate(')
end = s.index('  async function loadDataset()', start)
new_model = r'''  function capWeightShares(weights,maxShare=MAX_NEIGHBOR_WEIGHT_SHARE){
...
s = replace_between(s, '  function injectStatsCards(){', '  function ensureKnnControls(){', r'''  function injectStatsCards(){
...
s = replace_between(s, '  function ensureKnnControls(){', '  function resetStats(', r'''  function ensureKnnControls(){
...
s = replace_between(s, '  function resetStats(', '  function renderKnn(', r'''  function resetStats(message='—'){
...
s = replace_between(s, '  function renderKnn(', '  function currentTargetLotArea(){', r'''  function renderKnn(result,targetLotArea,category,radiusKm){
    injectStatsCards();
    ensureKnnControls();
    if(!result){
      resetStats();
...
r = Path('services/geometry-api/web/recovery-monitor.js')
rs = r.read_text(encoding='utf-8').replace('/static/market-knn.js?v=2.5.14-knn1','/static/market-knn.js?v=2.5.14-knn2')
r.write_text(rs, encoding='utf-8')

Path('services/geometry-api/web/market-knn.test.js').write_text(r'''const assert = require('assert');
